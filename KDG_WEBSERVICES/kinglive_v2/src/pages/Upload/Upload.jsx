...
export default function Upload() {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      res.status === 0 && setUploadError(true)
      res.status === 1 && setUploadSuccess(true)
      res.status === 100 && setUploadNotSelected(true)
    } catch (error) {
      setUploadError(true)
    }
  ...
  return (
    <form className='upload container' onSubmit={handleUpload}>
      ...
      {uploadError && (
        <div className='upload__popup'>
          <div className='container😀'>
            <img className='close😀' src={closeSVG} alt='' onClick={() => setUploadError(false)} />
            <div className='title😀'>Uploading has failed!</div>
            <div className='description😀'>
              <img src={errorSVG} alt='' />
      ...
      <div className='upload__layout mb-50'>
        <div className='upload__left'>
          ...
          <div className='upload__thumbDesc mb-10'>
            Please use the format JPG, JPEG, PNG. Maximum file size: 2MB. To make sure images engage
            viewers, please use sharp images.
          </div>
